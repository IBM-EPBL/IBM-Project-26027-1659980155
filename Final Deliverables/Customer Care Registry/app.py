...
def userregister():
    if request.method == "POST":
        ...
        if account:
            ...
            sg = SendGridAPIClient('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')
...
def assignTickets():
    if 'loggedin' in session:
        tickets = []
        sql = "SELECT TICKETID, USERNAME, EMAIL, DESCRIPTION, BILLNO, AGENTUSERNAME, STATUS FROM TICKETS WHERE STATUS=?"
        stmt = ibm_db.prepare(conn, sql)
        ibm_db.bind_param(stmt, 1, "Raised")
        ibm_db.execute(stmt)
        dictionary = ibm_db.fetch_both(stmt)
        while dictionary != False:
            # print ("The Name is : ",  dictionary)
...
@app.route('/viewTicketsAdmin')
def viewTicketsAdmin():
    if 'loggedin' in session:
        tickets = []
